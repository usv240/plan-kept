...
def test_full_http_collaboration():
 assert client.get("/").status_code==200;w=post("/api/workspaces").json();wid=w["workspace_id"];post(f"/api/workspaces/{wid}/open-perspectives");post(f"/api/workspaces/{wid}/demo-perspectives");w=post(f"/api/workspaces/{wid}/synthesize").json();assert w["ledger"][0]["state"]=="conflicting";post(f"/api/workspaces/{wid}/clarification",{"answer":"The access log confirms the room was unavailable until 10:25.","facilitator":"Riley Shah - synthetic"});post(f"/api/workspaces/{wid}/repair",{"decision":"implementation_gap","facilitator":"Riley Shah - synthetic"});post(f"/api/workspaces/{wid}/followup");w=post(f"/api/workspaces/{wid}/confirm",{"experienced":True,"note":"The room was available this time."}).json();assert w["status"]=="closed"
